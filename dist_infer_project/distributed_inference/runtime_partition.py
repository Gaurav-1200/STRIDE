...
class RuntimePartition(nn.Module):
    ...
    def forward_partition(
        ...
        *,
        ...
        # 1. Setup Hidden States
        if hidden_states is None:
            if self.embed_tokens is None:
                raise ValueError("First partition must have embeddings.")
            hidden_states = self.embed_tokens(input_ids.to(self.device))
        else:
            hidden_states = hidden_states.to(self.device, dtype=self.dtype)

        # 2. Setup Mask and Positions
        if attention_mask is not None:
            attention_mask = attention_mask.to(self.device, dtype=self.dtype)
            # 2. Convert [Batch, Seq] mask to [Batch, 1, Seq, Seq] causal mask
            # This prevents the model from "cheating" and looking at future tokens
            # and fixes the "1 1 1" gibberish output.
            batch_size, seq_len = hidden_states.shape[0], hidden_states.shape[1]
            
            # Create a square causal mask (lower triangular)
            causal_mask = torch.full((seq_len, seq_len), fill_value=torch.finfo(self.dtype).min, device=self.device,dtype=self.dtype)
            causal_mask = torch.triu(causal_mask, diagonal=1)
            
            # Combine with your passed attention_mask (in case there is padding)
            # [1, seq] -> [1, 1, 1, seq]
            extended_mask = attention_mask.view(batch_size, 1, 1, seq_len)
            # If mask is 1 (valid), add 0. If mask is 0 (invalid), add -inf.
            extended_mask = (torch.tensor(1.0, dtype=self.dtype, device=self.device) - extended_mask) * torch.finfo(self.dtype).min

    
            # Final 4D mask: [Batch, 1, Seq, Seq]
            attention_mask = causal_mask + extended_mask
        ...
        # 4. SINGLE LOOP (Process each layer exactly once)
        for idx in self.meta.layer_indices:
